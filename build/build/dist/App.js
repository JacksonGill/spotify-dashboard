...
const App = () => {
  ...
  useEffect(() => {
    (async () => {
      if (window.localStorage.getItem("spotify-dashboard-id")) {
        const savedData = JSON.parse(window.localStorage.getItem("spotify-dashboard-id"));
        const retreievedData = await spotifyService.initializeLocalStorageData(savedData);
  ...
  useEffect(() => {
    if (data.length === 0) {
      ...
      window.localStorage.setItem("spotify-dashboard-id", JSON.stringify(localStorageObject));
    }
  }, [data]);
  ...
  const deleteEntry = (view) => {
    ...
    if (data.length > 1) {
      if (data.findIndex((d) => d.id === view.id) <= indexTracker && indexTracker !== 0) {
        setIndexTracker(indexTracker - 1);
      }
    }
    setData(updatedData);
  ...
  }, errorMessage !== null ? errorMessage : ""), /* @__PURE__ */ React.createElement(MainView, {
    data,
    whatIsCurrentView,
    indexTracker,
    handleIndexChange
  ...
  }, "- Latest Updates and Releases -"), /* @__PURE__ */ React.createElement(Dashboard, {
    data,
    currentView,
    deleteEntry,
    changeView
  }), isDefault ? /* @__PURE__ */ React.createElement("p", {
    className: "default"
  }, "* You have not selected any of your favorite artists - the default view is displayed -") : null, !isEmpty(currentView) ? /* @__PURE__ */ React.createElement(AdditionalView, {
    currentView
  }) : "", /* @__PURE__ */ React.createElement(ExtraInfo, null), /* @__PURE__ */ React.createElement("div", {
    className: "seperator"
  }));
};
export default App;
